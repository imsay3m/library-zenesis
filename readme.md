...
14. psycopg2 installation for PostgreSql Local Server
...
16. Connect Your Database Onrender
    - <code>pip install dj-database-url dj-database-url psycopg2-binary</code>
    - In settings.py add <code>import dj_database_url</code>
    - <code>DATABASES={
        <br>'default': dj_database_url.config(default='external database url')
        <br>}</code>
    - Add External Database URL
    - Store Installed Packages <code>pip freeze > requirements.txt</code>
    - In settings.py add <code>ALLOWED_HOSTS = ["*"]</code>
    - <code>CSRF_TRUSTED_ORIGINS = ["https://yoururl.onrender.com", "https://*.127.0.0.1"]</code>